...
%-------------------------------------------------------------------------%
    ...
    methods(Access = public)
        ...
        function set_dic_params(obj, window_radius, sub_spacing) % CUSTOM
            % This function sets DIC parameters programmatically
                % Inputs: 
                %   subset_radius - integer (e.g., 20)
                %   spacing - integer (e.g., 5)
                %   cutoff - decimal (e.g., 0.0)
            

                obj.data_dic.dispinfo.radius = window_radius;
                obj.data_dic.dispinfo.spacing = sub_spacing;
                obj.data_dic.dispinfo.cutoff_diffnorm = 1e-06;
                obj.data_dic.dispinfo.cutoff_iteration = 50;
                obj.data_dic.dispinfo.total_threads = 8;
                obj.data_dic.dispinfo.stepanalysis(1).enabled = false;
                obj.data_dic.dispinfo.stepanalysis.type = 'seed';
                obj.data_dic.dispinfo.stepanalysis.auto = true;
                obj.data_dic.dispinfo.stepanalysis.step = 5;
                obj.data_dic.dispinfo.subsettrunc = false;
                
                % 3. Update the GUI state so the next step (Analysis) becomes enabled
                update_dicstatetext(obj)
                update_topmenu(obj)

                % 4. Optional: Log completion to console
                disp('DIC Parameters set successfully via script.');
        end

        function run_dic_auto(obj) % CUSTOM
            % This simulates clicking the "Perform DIC Analysis" button
            % It checks if the button is enabled first (sanity check)
            if strcmp(get(obj.handles_gui.topmenu_perfdic, 'Enable'), 'on')
                % Call the callback directly
                obj.callback_topmenu_dic(obj.handles_gui.topmenu_perfdic, []);
            else
                error('Cannot run DIC: Parameters or ROI/Seeds are not set yet.');
            end
            update_dicstatetext(obj)
            update_topmenu(obj)
        end

        function close_auto(obj) % CUSTOM
    ...
    methods(Access = public)
        ...
        % This is the callback for performing DIC.
            ...
            if (overwrite)
                ...
                % CUSTOM %msgbox('DIC Analysis completed successfully. Press ok to finish.','WindowStyle','modal');
            end    
        end
        ...
        function callback_close_function(obj,hObject,eventdata,force) %#ok<INUSL>
            ...
            if (~force && ~isempty(obj.data_dic.dispinfo) && ~isempty(obj.data_dic.displacements))
                contbutton = 'Yes'; % CUSTOM % questdlg('Prior DIC data has been detected. If you continue without saving, this data will be deleted. Do you want to proceed?','Continue Operation','Yes','No','Yes');
                if (strcmp(contbutton,'Yes'))
                    closencorr = true;
                end
            else
