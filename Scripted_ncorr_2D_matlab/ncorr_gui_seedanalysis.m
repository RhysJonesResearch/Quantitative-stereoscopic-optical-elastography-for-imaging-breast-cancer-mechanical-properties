...
% all images will be seeded if outstate is set to success. 
    ...
    function callback_button_selectregion(hObject,eventdata) %#ok<INUSD>
        ...
        while (~regionfinished)
            % Initialize point
            handle_point = impoint(handles_gui.axes_selectregion,glob_col_m,glob_row_m);  %[440,224]; %impoint(handles_gui.axes_selectregion);  % CUSTOM  need to find out what format for point is correct  
            ...
            if (~isempty(handle_point))          
                ...
                pos_region = [glob_col_m, glob_row_m]; %round(getPosition(handle_point))-1; % CUSTOM
    ...
    drawnow;
    callback_button_selectregion(handles_gui.button_selectregion, []);
    drawnow;
    callback_button_finish(handles_gui.button_finish, []);
    drawnow;
    if ishandle(handles_gui.figure)
