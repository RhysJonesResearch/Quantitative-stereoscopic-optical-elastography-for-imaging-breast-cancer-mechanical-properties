...
% if outstate returns true.
    ...
    function callback_button_setseeds(hObject,eventdata) %#ok<INUSD>
        ...
        for i = 0:total_threads-1
            ...
            seedfinished = false;                
            while (~seedfinished)   
                custom_point = glob_seeds(i+1,:); % CUSTOM
                %disp(custom_point)
                handle_point_buffer = impoint(handles_gui.axes_setseeds,custom_point(1),custom_point(2));  
    ...
    % CUSTOM
    drawnow;
    callback_button_setseeds(handles_gui.button_setseeds, []);
    drawnow;
